...
import fetchYoutubeVideoInfo from "./fetchYoutubeVideoInfo.js";
...
const seed = async () => {
  ...
  if (clipCount > 0) {
    return { message: "Already have records, delete them before seed." };
  }

  const seedCount = 6;

  const allVideos = getBaseVideos();
  const baseVideoData = {};
  Object.keys(allVideos)
    .sort(() => Math.round(Math.random()))
    .slice(0, seedCount - 1)
    .forEach((videoId) => {
      baseVideoData[videoId] = allVideos[videoId];
    });
  ...
  return await Clip.create(clipData);
};
